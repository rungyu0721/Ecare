...
def simple_extract(text: str) -> Extracted:
    ...
    if has_child_protection_signal or has_explicit_violence_signal or has_weapon_threat_context or (
        ...
        ex.category = "暴力事件"
    elif v4_category in ["交通事故", "可疑人士", "火災", "天然災害", "受困救援", "自殺危機", "失蹤走失", "暴力事件"]:
        ...
        ex.category = "火災"
    elif any(k in text for k in ["地震", "強震", "餘震", "颱風", "豪雨", "水災", "淹水", "積水", "土石流", "坡地崩塌", "坍方", "建築物倒塌", "房屋倒塌", "大樓倒塌", "橋斷", "道路中斷"]):
        ex.category = "天然災害"
    elif any(k in text for k in ["電梯受困", "困在電梯", "卡在電梯", "電梯卡住", "電梯故障", "電梯停住", "電梯打不開", "出不了電梯", "出不來電梯"]):
        ex.category = "受困救援"
    elif any(k in text for k in ["自殺", "想死", "不想活", "輕生", "尋短", "跳樓", "要跳樓", "準備跳樓", "站在頂樓", "站在陽台外", "割腕", "吞藥", "吃藥自殺", "燒炭", "上吊"]):
        ex.category = "自殺危機"
    elif any(k in text for k in ["失蹤", "走失", "找不到人", "找不到小孩", "找不到老人", "老人走失", "小孩走失", "小孩失蹤", "長輩走失", "家人失聯", "聯絡不上", "不見了"]):
        ex.category = "失蹤走失"
        ...
        for k in [
            ...
            "喘不過氣", "吸不到氣", "不舒服", "發燒", "嘔吐",
            "OHCA", "到院前心肺功能停止", "心肺功能停止", "心跳停止", "心跳呼吸停止",
            "中風", "腦中風", "疑似中風", "心肌梗塞", "心臟病發作",
    ...
    injury_terms = ["流血", "大量流血", "血流不停", "受傷", "燙傷", "燒傷", "灼傷", "燙到", "燒到", "水泡", "昏倒", "暈倒", "暈過去", "昏過去", "倒地", "倒下", "倒在地上", "倒在路邊", "沒反應", "沒有反應", "無反應", "叫不醒", "沒呼吸", "抽搐", "骨折", "頭暈", "胸痛", "胸悶", "心臟痛", "半邊無力", "嘴歪", "講話不清楚", "呼吸困難", "喘不過氣", "吸不到氣", "嘔吐", "噎到", "哽塞", "噎住", "臉發紫", "中暑", "熱衰竭", "癲癇", "失溫", "高山症", "脫水", "蛇咬", "蜂螫", "墜落", "摔落", "滑落", "墜谷", "被水沖走", "被沖走", "沖走", "被壓住", "有人被埋", "瓦礫壓住", "困在裡面", "出不來", "不能走", "無法走", "無法行走", "走不動", "OHCA", "到院前心肺功能停止", "心肺功能停止", "心跳停止", "心跳呼吸停止", "中風", "腦中風", "疑似中風", "心肌梗塞", "心臟病發作", "割腕", "吞藥", "吃藥自殺", "AED", "CPR", "胸外按壓", "救護車", "沒有在呼吸", "沒在呼吸"]
    ...
    ex.danger_active = True if has_child_protection_signal or any(k in text for k in ["還在", "持續", "正在", "還沒結束", "還在現場", "追人", "追我", "揮刀", "攻擊", "毆打", "攻擊中", "迷路", "迷途", "受困", "失聯", "溪水暴漲", "溪水變大", "溪水變急", "水位上升", "被水沖走", "被沖走", "沖走", "卡在對岸", "過不了溪", "過不了河", "漂走", "水變深", "渡溪失敗", "坍方", "落石", "土石流", "地震", "強震", "還在搖", "餘震", "淹水", "積水", "建築物倒塌", "房屋倒塌", "大樓倒塌", "橋斷", "道路中斷", "被壓住", "有人被埋", "瓦礫壓住", "停電", "電梯受困", "困在電梯", "卡在電梯", "電梯卡住", "電梯打不開", "自殺", "想死", "不想活", "輕生", "跳樓", "站在頂樓", "站在陽台外", "割腕", "吞藥", "燒炭", "上吊", "失蹤", "走失", "找不到", "聯絡不上", "不見了", "手機快沒電", "手機沒電", "快沒電", "電量不足", "剩一格電", "只剩一格電", "剩5%", "剩 5%", "剩不到10%", "剩不到 10%", "沒訊號", "沒有訊號", "訊號不好", "定位跑掉", "GPS不準", "GPS 不準", "找不到座標", "不知道座標", "沒有座標", "下大雨", "大雨", "起霧", "濃霧", "氣溫很低", "低溫", "很冷", "天黑", "天色變暗", "不能走", "無法走", "無法行走", "走不動"]) else None
...
def apply_turn_context(messages: List[ChatMessage], ex: Extracted) -> Extracted:
    ...
    if ex.category == "待確認" and latest_user_text_val:
        category_map = {
            ...
            "電梯受困": "受困救援", "自殺": "自殺危機", "跳樓": "自殺危機", "失蹤": "失蹤走失", "走失": "失蹤走失",
        }
