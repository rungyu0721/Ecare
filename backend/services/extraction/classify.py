...
CATEGORY_QUESTION_KEYWORDS = ["火災", "天然災害", "受困救援", "自殺危機", "失蹤走失", "可疑人士", "噪音", "醫療急症", "山域水域救援", "暴力事件", "交通事故", "發生了什麼事"]
...
def get_dispatch_advice(
    ...
    if category == "受困救援":
        return "建議派遣：消防救援，若有人不適同步救護車"
    if category == "自殺危機":
        return "建議派遣：救護車 + 警察"
    if category == "失蹤走失":
        return "建議派遣：警察，若在山區水域或有人受傷同步 119"
    if category == "醫療急症":
        return "建議派遣：救護車"
    if category == "山域水域救援":
        return "建議派遣：消防救援，必要時同步救護車"
    if category == "暴力事件":
...
def should_ask_scene_danger(ex: Extracted, risk_level: str) -> bool:
    if ex.danger_active is not None or risk_level not in ["Medium", "High"]:
        return False
    return ex.category in ["火災", "天然災害", "受困救援", "自殺危機", "失蹤走失", "暴力事件", "交通事故", "可疑人士", "山域水域救援"]


def build_incident_acknowledgement(ex: Extracted) -> str:
    if ex.category == "醫療急症":
        return "收到，現場有人身體不舒服，我先幫你整理。"
    if ex.category == "山域水域救援":
    ...
    if ex.category == "受困救援":
        return "收到，這比較像受困救援情境，我先幫你整理位置、受困人數與是否有人不適。"
    if ex.category == "自殺危機":
        return "收到，這是高風險自殺危機，我先幫你整理位置與立即安全狀況。"
    if ex.category == "失蹤走失":
        return "收到，這比較像失蹤或走失通報，我先幫你整理最後出現位置與特徵。"
    if ex.category == "暴力事件":
