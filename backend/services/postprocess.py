...
from backend.services.incident_taxonomy import has_remote_rescue_signal, is_remote_rescue_extracted
from backend.services.event_update import apply_event_update_response
...
HYPOTHERMIA_TERMS = ["失溫", "體溫過低"]
ALTITUDE_SICKNESS_TERMS = ["高山症", "高山反應", "高地反應"]
WATER_RESCUE_TERMS = ["被水沖走", "被沖走", "沖走了", "溪水暴漲", "水位上升", "卡在對岸", "過不了溪", "過不了河", "渡溪失敗"]
...
def _is_remote_rescue_context(text: str, ex: Extracted) -> bool:
    return (
        ex.category == "山域水域救援"
        or is_remote_rescue_extracted(ex.symptom_summary)
        or has_remote_rescue_signal(text)
    )
...
def first_aid_guidance_for_text(text: str, ex: Extracted) -> Optional[tuple[str, str]]:
    ...
    remote_rescue = _is_remote_rescue_context(text, ex)
    ...
    if has_any_term(text, HYPOTHERMIA_TERMS):
...
def contextualize_reply_and_question(
    ...
    event_update_result = apply_event_update_response(ex, latest_user_text, risk_level)
    if event_update_result:
        return event_update_result
    ...
    if (
        ...
        if next_question_is_generic or asks_about_location(next_q):
            if ex.category == "待確認":
                next_q = "那現場現在是發生了什麼事？像是火災、天然災害、電梯受困、自殺危機、走失、衝突、車禍，還是有人身體不舒服？"
            else:
                next_q = next_question(ex, risk_level)
        ...
        and ex.category != "待確認"
        and latest_user_text
        and any(kw in previous_assistant_text for kw in ["火災", "天然災害", "受困救援", "自殺危機", "失蹤走失", "可疑人士", "噪音", "醫療急症", "暴力事件", "交通事故"])
