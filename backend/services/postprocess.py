...
def _apply_event_update_response(
    ex: Extracted,
    ...
    if category == "受困救援":
        ...
        if trapped or discomfort or released:
            ...
            set_dispatch()
            if released:
                return reply, "有人受傷、不舒服、喘不過氣，或需要救護車嗎？"
            if ex.location:
    ...
    if category == "火災":
        ...
        if active_fire or fire_out or trapped_or_injured:
            ...
            return reply, next_question(ex, risk_level)
...
def contextualize_reply_and_question(
    ...
    event_update_result = _apply_event_update_response(ex, latest_user_text, risk_level)
